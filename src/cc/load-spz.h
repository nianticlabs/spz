...
// Represents a single inflated gaussian. Each gaussian has 236 bytes. Although
// the data is easier to intepret in this format, it is not more precise than
// the packed format, since it was inflated.
...
// Represents a single low precision gaussian. Each gaussian has exactly 64
// bytes, even if it does not have full spherical harmonics.
struct PackedGaussian {
  ...
  UnpackedGaussian unpack(bool usesFloat16, int32_t fractionalBits,
                          const CoordinateConverter &c, float shMin = -1.0f,
                          float shMax = 1.0f) const;
};

// Represents a full splat with lower precision. Each splat has at most 64
// bytes, although splats with fewer spherical harmonics degrees will have less.
...
struct PackedGaussians {
  ...
  int32_t fractionalBits =
      0; // Number of bits used for fractional part of fixed-point coords
  bool antialiased =
      false; // Whether gaussians should be rendered with mip-splat antialiasing
...
bool saveSpz(const GaussianCloud &gaussians, const PackOptions &options,
...
bool saveSplatToPly(const spz::GaussianCloud &gaussians,
                    const PackOptions &options, const std::string &filename);
